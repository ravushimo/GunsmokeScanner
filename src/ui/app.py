...
class GunsmokeApp:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Gunsmoke Scanner - Refactored")
    ...
    def setup_ui(self):
        ...
        # Logo
        tk.Label(header, text="⚡ Gunsmoke Scanner", font=("Segoe UI", 18, "bold"),
                bg=THEME['bg_medium'], fg=THEME['text_primary']).pack(side=tk.LEFT, padx=20)
        
        # Right controls (simplified)
        controls = tk.Frame(header, bg=THEME['bg_medium'])
        controls.pack(side=tk.RIGHT, padx=20)
        
        # Always on top
        self.top_var = tk.BooleanVar(value=True)
        tk.Checkbutton(controls, text="Always on Top", variable=self.top_var,
        ...
        tk.Checkbutton(controls, text="Show Overlay", variable=self.overlay_var,
